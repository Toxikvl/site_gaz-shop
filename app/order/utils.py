...
class OrderCreator(object):
    ...
    def place_order(self, basket, total,  # noqa (too complex (12))
        ...
        # send_mail(
        #     subject=f'Поступил заказ № {order_number}',
        #     message=f'Поступил заказ  {order} - {order.user}\n',
        #     html_message='',
        #     from_email='admin@avtogazkomplekt.ru',
        #     recipient_list=['ravenvl71@gmail.com'],
        #     fail_silently=False,
        # )
        try:
...
class OrderDispatcher:
    ...
    def send_order_placed_email_for_user(self, order, extra_context, attachments=None):
        ...
        send_mail(
            ...
            message='',
            html_message=messages['html'],
            from_email=getattr(settings, 'OSCAR_FROM_EMAIL'),
            recipient_list=getattr(settings, 'RECIPIENTS_EMAIL'),
            fail_silently=False,
        )
