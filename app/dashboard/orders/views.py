...
class OrderListView(BulkEditMixin, ListView):
    ...
    def download_selected_orders(self, request, orders):
        ...
        for order in orders:
            ...
            str_number += 1
            worksheet.write(f'A{str_number}', 'Код')
            worksheet.write(f'B{str_number}', 'Наименование ')
            worksheet.write(f'C{str_number}', 'Кол. ')
            worksheet.write(f'D{str_number}', 'Сумма')

            for line in list(order._prefetched_objects_cache['lines']):
                str_number += 1
                worksheet.write(f'A{str_number}', line.upc)
                worksheet.write(f'B{str_number}', line.title)
                worksheet.write(f'C{str_number}', str(line.quantity))
                worksheet.write(f'D{str_number}', str(line.unit_price_excl_tax))
            str_number += 1

        workbook.close()
        fsock = open(path_excel, "rb")
